...
def tokenize_and_pad(texts, max_len=400, vocab_size=20000):
...
# Build RNN model with tunable hyperparameters
def build_model(hp):
    model = Sequential()
    model.add(Embedding(input_dim=20000, output_dim=hp.Int('embedding_dim', 100, 200, step=50)))
    model.add(SimpleRNN(hp.Int('rnn_units', 64, 256, step=64), return_sequences=False))
